...
Most languages (both high and low-level) provide some sort of [[http://en.wikipedia.org/wiki/Foreign_function_interface][foreign
...
involves creating an xtlang 'header',[fn:header] which creates xtlang
...
But hang on a sec---if =foo= is a C function, then why are we telling
Extempore that it's a pointer to a /closure/? Have a look, the type of
=foo= declared in the =bind-lib= form is a closure pointer
=[i64,i64]*=. And an xtlang closure is similar, but not the same as a
C function (see [[file:2012-08-09-xtlang-type-reference.org][this post]] for more detail on the differences between
xtlang closures and plain function).

The reason for this is that xtlang doesn't really deal with functions,
it always deals with closures (todo link). Because closures are 'first
class'[fn:fc] in xtlang, we /always/ work with closures in xtlang. A
lot of the time closures behave just like 'pure' functions: they take
arguments and return a value, a given input will always give rise to
the same output, and no side effects will be triggered. But closures
can also maintain state because they 'capture' all the variables they
reference.  Often this is really handy, and lots of xtlang code you'll
see in the examples directories does this sort of thing.

So, what =bind-val= does when you declare =foo= as having the type
=[i64,i64]*= is create a closure which calls the C function =foo=
declared in =foo.c=.[fn:get-native] It's this /foo/ closure that you
use in your xtlang code, and it makes no difference that it was
originally defined in a shared library, you can use it just like any
xtlang closure. You can even redefine =foo= as a pure xtlang closure
to change the functionality of your program, as long as you don't
...
the excellent KissFFT library for doing [[http://en.wikipedia.org/wiki/Fourier_transform][Fourier transforms]]. The
...
source, build the =kiss_fft.dylib= library and put it somewhere that
...
Ah, Extempore can't create an xtlang closure for =kiss_fft= without
knowing about all the types it refers to. So, let's dive back into the
...
So we need to do that before we can tell Extempore about the
...
appropriate types and closures, the above code finally compiles:
...
haven't included in this post: a couple of helper functions for
setting up the =kiss_fft_cfg= struct, determining efficient FFT stride
lengths and other things like that. You don't have to =bind-lib= all
the functions in the library, just the ones you need, although knowing
which ones sometimes more of an art than a science. If the library has
a well defined [[http://en.wikipedia.org/wiki/Application_programming_interface][API]] then it might be clear exactly how to get what you
want out of the library, but sometimes it just takes a bit of digging
...
like you could write yourself, and if you want to change any of the
closures defined in these libraries you can do it on the fly, just as
you can with any other xtlang code.  This is pretty cool, there's
something exciting about being able to hack on the standard library
...
[fn:fc] 'First class' closures means that xtlang closures can be used
just like values of any other type. They can be assigned to variables,
stored in tuples and arrays, and passed to and returned from other
closures.

[fn:get-native] You can actually get the native C function associated
with any closure using =llvm:get-native=, but this is only necessary
if you need to do something like provide a function pointer to an
external C library you're trying to use. In xtlang, the closure is all
there is or ever was or ever will be (with apologies to Carl Sagan).
