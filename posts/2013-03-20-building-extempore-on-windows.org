...
When you're done, copy the resulting =pcre.dll= into
=C:\Windows\System32=.
...
#+begin_src sh
bootstrap.bat
bjam --build-dir=<boost_build_dir> toolset=msvc link=static address-model=64 variant=release --build-type=complete --with-thread --with-date_time --with-filesystem --with-system --with-regex stage
#+end_src
...
#+begin_src sh
boost_build_dir\libboost_thread-vc120-mt-1_47.lib
#+end_src
