...
Extempore can run both scheme and xtlang code but xtlang's reference
types (tuples,array, vectors, closures) are simply opaque pointers to
scheme. scheme can pass these pointers around. getting them as the result
of one xtlang closure and passing them off as an argument to another,
but scheme itself doesn't know how to do much else with them.
Primitive types such as =double= and =i64= do not have this limitation.
Here's an example to make things a bit clearer:

#+begin_src extempore
;; tuple-maker return a tuple and tuple-taker takes one as an argument.
...
(tuple-taker (tuple-maker)) ; returns 42. scheme *can* pass tuples around

#+end_src

Have a look at =extampore_lang.xtm= for more examples.
