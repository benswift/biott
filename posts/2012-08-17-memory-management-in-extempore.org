...
sluggishness (and nondeterminism) of Impromptu's Scheme interpreter
...
one was slow.  The nondeterminism was even more of a problem, because
...
xtlang. String literals are bound globally,[fn::see [[file:~/Documents/biott/posts/2012-08-09-xtlang-type-reference.org][here]] for more info about strings in
xtlang] but that's the exception to the rule---everything else which
is bound in a =let= inside an xtlang =lambda= will be stack allocated,
unless you explicitly request otherwise with =halloc= or =zalloc=.

This works for int and float primitives because there exist float and
int literals which are understood by the compiler, but how about
aggregate and other higher-order types?  In those cases, we can call
=salloc= explicitly.
...
The reason for this is that the =salloc= (inside the closure
=double_tuple=) which allocated the memory for us doesn't keep the
memory after the closure returns.  Subsequent calls to the closure
can (and will) overwrite this memory, and any pointers to it (which is
what we have in =tup=: the pointer returned by =(double_tuple 6)=)
will have their values changed 'without permission'.  That's what
deallocating memory means: it doesn't mean that the memory gets set to
zero, or that new values will be written in straight away, but it
means that new values /might/ be written in at any stage.  Which, from
a programming perspective, is just as bad as having new data written
into it, because if you can't trust that your pointer still points to
the value(s) you think it does then it's pretty useless as a pointer.
...
going to be allocing a whole lot of objects for a specific algorithmic
