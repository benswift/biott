...
OpenGL, Cairo, Bullet, etc' (which live under =libs/external=).
While =extempore= can and will compile these dynamically when you tell
it to load them, you can speed things up considerably by precompiling
them just once beforehand using the provided shell scripts.
Running =compile-stdlib.sh= will invoke the =extempore= compiler and generate
a shared library + an xtm wrapper file in the the =libs/= directory.
When you next tell =extempore= to load the library
(using =(sys:load "libs/foo/bar.xtm")=) it will try to load the precompiled
library directly from =libs/= instead, which should be significantly faster.
...
but if you do, take a look at the associated source file for instructions
on obtaining the required dependencies.
...
[fn:Credit_G] Thanks to Giora Heller for contributing this section.
