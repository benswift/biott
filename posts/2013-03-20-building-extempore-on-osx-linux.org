...
The one (slightly) tricky dependency is LLVM v3.4, which requires a
...
v3.4. Unfortunately the bug in =LLParser.cpp= hasn't yet been fixed,
...
build LLVM manually (you can get the LLVM 3.4 source [[http://llvm.org/releases/3.4.1/llvm-3.4.1.src.tar.gz][here]]) because a
...
probably be =/usr/local/Cellar/extempore-llvm/3.4=.
