...
In this post we're going to make an Extempore instrument. Basically,
an instrument in xtlang is just a couple of xtlang closures: a the
note kernel (by convention ending in =-note=), and the FX (effects)
closure (by convention ending in =-fx=). When properly set up, an
instrument can be played like a midi soft-synth, so individual notes
can be triggered with an amplitude, a pitch and a duration. Impromptu
...
is that the whole signal chain is now written in xtlang, and you can
have a look at it in =libs/core/audio_dsp.xtm= if you want to see the
nuts and bolts of how it works.[fn:dsp-chain]
...
#+begin_src extempore
  ...
  ;; send the organ timbre to the audio sink
...
Compiling the function =organ-timbre= does three things:
...
When we call the =organ-timbre= closure in the =dsp= callback, we hear
a droning organ tone. It should be really obvious at this point that
the closure =organ-timbre= doesn't represent a /pure/ function: one
...
the =organ-timbre= closure to produce a nice pitched organ tone, there
...
one oscillator (and therefore incrementing its phase) doesn't affect
the phase of any other oscillators which might be floating around.
This is super handy, because it allows each oscillator to do its own
