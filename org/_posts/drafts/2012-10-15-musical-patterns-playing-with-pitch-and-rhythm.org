...
It may seem a little strange when you first look at impromptu that
...
some way influence its user my goal with impromptu is to provide a
framework without any prescribed musical features. Of course this is
somewhat of a ridiculous goal given that straight out of the gate
impromptu's use of MIDI strongly preferences a traditional diatonic
tonal system (I should point out that MIDI does not enforce this -
quarter tone composers don't despair, I'll do a quarter tone
example!). Nevertheless my aim with impromptu is primarily to provide
...
way to represent or control pitch in impromptu---you are just as free
to use a function to continuously control the Hz value of a Reaktor
patch parameter (as I often do). I designed impromptu so you don't
...
One last thing. Make sure you have read the impromptu basics tutorial
...
impromptu does not by default include pitch symbols) so instead of
...
discussed, impromptu does not include much in-built musical support,
...
implement this markov matrix in impromptu (if you don't know what
...
minor. Because impromptu symbols are lowercase only we use 'i for I 'v
...
Wind Quintet Number 1. By Impromptu progression_three.mp3 Bring back
the beat I hear you say. OK, onto Beat & Tempo.

So far we have been using Impromptu's default time standard, samples
...
default impromptu runs at 60 bpm or 1.0 = *samplerate*. We'll also add
