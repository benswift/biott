...
=libboost_thread-vc100-mt-1_47= (although not necessarily
/exactly/ the same) depending on your setup. Extempore /does/
...
For example, if building extempore fails because it cannot find the
boost thread library =libboost_thread-vc100-mt-1_47=, find the file

#+begin_example
my_boost_build\boost\bin.v2\libs\date_time\build\msvc\release\address-model-64\link-static\threading-multi\libboost_thread-vc-mt-1_47.lib
#+end_example

and copy it (note the slight name change) to

#+begin_example
my_boost_build\libboost_thread-vc100-mt-1_47.lib
#+end_example

You probably won't need to worry about this until you try and build
Extempore itself.
