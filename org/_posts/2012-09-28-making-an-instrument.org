...
of flanger closures[fn:flanger] (=flanl= and =flanr=) and a pair of oscillator
...
=define-instrument= to set everything up

#+begin_src extempore
  (define-instrument organ organ-note organ-fx)

  ;; TODO what's the compiler output?
#+end_src
...
#+begin_src extempore
  ...
  (play-note (now)    ; time
             organ    ; instrument
             440.0    ; frequency (Hz)
             1.0      ; amplitude (loudness)
             44100.0) ; duration (in samples, 44100.0 = 1sec)
#+end_src

if everything is hooked up right, when you evaluate (=C-x C-x=) the
=play-note= call above you should hear a 440Hz (the A below middle C)
organ note which plays for one second (44100 samples).  Try changing
the frequency, amplitude and loudness values and see how the note
changes.

If this /didn't/ work, there are a few possible causes:
