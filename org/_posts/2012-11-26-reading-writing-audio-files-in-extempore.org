...
will, when called, return successive [[file:2012-08-13-understanding-pointers-in-xtlang.org][pointers]] into the audio file in
such a way that =audiofile= can be called once per output sample and
will return pointers which 'scrub through' the audio file at normal
speed.  When =dsp= is compiled, the log view also prints some info
about the file:
...
/If you're interested, the file I'm using is 'Peg' from [[http://www.rollingstone.com/music/lists/500-greatest-albums-of-all-time-20120531/steely-dan-aja-20120524][Steely Dan's
    ...
    (let ((audiofile (audiofile_c "/Users/ben/Desktop/xtm-assets/peg.wav" 0 0))
      ...
      (lambda (in time chan dat)
        (* 0.1 ;; to compensate for saturation boost
           (cond ((= chan 0.0)
                  (saturationl (pref (audiofile) 0) 1.0 0.9))
                 ((= chan 1.0)
                  (saturationr (pref (audiofile) 0) 1.0 0.9))
                 (else 0.0))))))
#+end_src

Sounds messy---great.  Now, we want to write the processed audio back
to a wav file.  To do this, we need to [[file:2012-08-17-memory-management-in-extempore.org][allocate some memory]] to write
the output samples into.  Looking at the file info which was printed
out earlier (in particular =samples read: 21202944)=, we know that
there are =21202944= samples in the input file, so that's how big we
want our out buffer to be.
...
closure will be called with the output data pointer.

Sure enough, I hear the processed (saturated) output file when I
listen to the =peg-processed.wav= file.

There are lots of alternate ways to do this: the sound can be
processed offline (rather than in real-time inside the =dsp=
callback), and you can write your own functions to either simplify or
make the process more flexible. You can see the low-level libsndfile
functions involved if you look in =libs/external/sndfile.xtm=.

But the primary workflow is

1. write the audio samples to a buffer
2. write that buffer to a file using =write_audio_data=

and you're done---writing audio files with Extempore.
