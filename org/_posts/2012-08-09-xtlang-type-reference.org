...
detour into some memory stuff as well, so we'll cover some of that as
...
This info will probably go into the [[http://github.com/digego/extempore/wiki][wiki]] at some point, but for now
it's just an attempt to explain things, and to help people understand
and write working xtlang code.
...
In xtlang, [[*Type%20annotations][type annotations]] (a type definition following a colon in
the source code) are used to tell the compiler about the types of
variables. The compiler will [[*Type%20inferencing][infer]] incomplete type information (such
as the size of int and float literals), but will never silently coerce the
types. If there's a problem, the compiler will complain and you have
to fix it, either with an explicit coercion or by changing the
...
- there is no =char= type, only =i8=. [[**%20Strings%20in%20xtlang][Strings in xtlang]] are
  null-terminated char arrays/pointers with the signature =i8*=.

*** Floats

#+begin_html
<a href=""><img src="images/float-examples.png" alt=""></a>
#+end_html
...
#+begin_html
<a href=""><img src="images/pointer-examples.png" alt=""></a>
#+end_html
...
this post. For now, though, we'll just use =zalloc=, which allocates
...
can do some of the bookkeeping for you (more on this [[**%20Custom%20types][below]]).
...
#+begin_html
<a href=""><img src="images/vector-examples.png" alt=""></a>
#+end_html
...
#+begin_html
<a href=""><img src="images/closure-examples.png" alt=""></a>
#+end_html
...
The usual =pref= and friends for pointers (described [[**Pointer%20types][above]]) are
...
convenienct: it's easier to type =my_type= than
...
=bind-type= tells the LLVM compiler about your new type, which
...
LLVM compiler. =bind-alias= doesn't tell the compiler as much about
the code as =bind-type= does, which can lead to execution-time
problems which would otherwise have been caught by the compiler.  So
you should almost always use =bind-type= over =bind-alias=.
...
As usual, there are other places to look for more info: the file
=examples/core/extempore_lang.xtm= is a good overview of all these
types, and includes heaps of examples as well.  And there's always the
project page and source code at [[https://github.com/digego/extempore][github]].

# things to ask Sorryballs:
# function closure?
# aggregate types - good name?
# 'array' as a name for the chunk of memory returned by alloc
